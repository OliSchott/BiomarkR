...
#' @param filepath The path to the dataset
#' @param programm The program used to generate the data c("DIA-NN, "Spectronaut)
#' @param feature The feature to be imported (Protein or Peptide)
#' @return A dataset with the imported data
#' @export
ImportMSData <- function(filepath, feature = "Protein", programm = "DIA-NN") {
  ...
  ## Reading Spectronaut output file
  if (programm == "Spectronaut" | programm == "spectronaut"){

    if(feature == "Protein" | feature == "protein"){

      dataset <- arrow::read_parquet(filepath)

      dataset <- dataset %>%
        dplyr::mutate(Protein = paste0(protein_accession,"_" ,gene_id)) %>%
        ## deselect the first 5 columns
        dplyr::select(-c(1:5)) %>%
        tidyr::pivot_longer(cols = contains("_pg_"), names_to = "Sample", values_to = "Intensity") %>%
    ...
    if(feature == "Peptide" | feature == "peptide"){
      ...
      stop("Peptide level data import from Spectronaut not supported yet")
...
TTest <- function(dataset, plotname = "", method = "unsupervised", clustDist = "euclidean", p.adj.method = "BH") {
  ...
  if("Protein" %in% colnames(dataset)) {
    ...
    vulcanoPlot <- ggplot2::ggplot(data = VulconaoPlotData) +
      ...
      ggplot2::xlab(paste("Log2 Fold change in", unique(TResults$group1))) +
...
WTest <- function(dataset, plotname = "", method = "unsupervised", clustDist = "euclidean", p.adj.method = "BH", ColPalette = "custom_vibrant") {
  ...
  if("Protein" %in% colnames(dataset)) {
    ...
    ## Create dataframe of significant Proteins
    ...
    vulcanoPlot <- ggplot2::ggplot(data = VulconaoPlotData) +
      ...
      ggplot2::ggtitle(plotname) +
      ggplot2::xlab(paste("Log2 Fold change in", unique(WResults$group1))) +
      ggplot2::theme_light(base_size = 13) +
      ## rename y axis
      ggplot2::ylab(ifelse(p.adj.method != "none", "log10 (p.adj)", "log10 (p)"))
  }
  ...
  if("Protein" %in% colnames(dataset)){
    ...
    Heatmap <- BiomarkR::HeatMap(datasetW %>% dplyr::select(c(Sample,Protein,Intensity,Status)), PoIs = WilcoxSignificantFeatures$Protein, method = method, clustDist = clustDist, show_column_names = F, show_row_names = F, plotname = plotname, ColPalette = ColPalette)
  ...
  if("Peptide" %in% colnames(dataset)){
    ...
    Heatmap <- BiomarkR::HeatMap(datasetW %>% dplyr::select(c(Sample,Peptide,Intensity,Status)), PoIs = WilcoxSignificantFeatures$Peptide, method = method, clustDist = clustDist, show_column_names = F, show_row_names = F,plotname = plotname, ColPalette = ColPalette)
...
KeggEnrichmetn <- function(dataset, PoIs, folder = NA, plotname = ""){
  ...
  PathwayData <- WtestResults$raw %>%
    ...
    ## round estimate
    dplyr::mutate(estimate = base::round(estimate, 2)) %>%
...
MEGENA <- function(dataset, plotname = ""){
  ...
  TestData <- dataset %>%
    ...
    tidyr::pivot_wider(names_from = Protein, values_from = Intensity) %>%
    tidyr::pivot_longer(cols = -Sample, names_to = "Protein", values_to = "Intensity") %>%
    BiomarkR::ImputeFeatureIntensity() %>%
    tidyr::pivot_wider(names_from = Protein, values_from = Intensity) %>%
    tibble::column_to_rownames(var = "Sample") %>%
    as.matrix() %>% t()
